...
import { getWrapped } from '@pages/api/v1/restock/wrapped';
...
const Wrapped2025 = (props: Wrapped2025Props) => {
...
export async function getServerSideProps(context: any) {
  ...
  try {
    ...
    const data = await getWrapped(res.user.id, 2025);
    ...
    return {
      props: {
        ...
        user: res.user,
        messages: await loadTranslation(locale as string, 'restock/dashboard/2025'),
      },
    };
  } catch (e) {
    if (typeof e !== 'string') {
...
Wrapped2025.getLayout = function getLayout(page: ReactElement, props: any) {
  const t = createTranslator({ messages: props.messages, locale: props.locale });
  const canonical = 'https://itemdb.com.br/restock/dashboard/2025';

  return (
    <Layout
      SEO={{
        title: '2025 Restock Review',
...
const errorCodeToMessage = (code: string) => {
  switch (code) {
      ...
      return (
        <>
          ...
          <Text fontSize={'sm'} opacity={0.87} mt={3}>
            ...
            <Link asChild color="#D2FF72">
              <MainLink href="/feedback/trades" prefetch={false}>
                Trade Pricing
              </MainLink>
            </Link>{' '}
            or{' '}
            <Link asChild color="#D2FF72">
              <MainLink href="/feedback/vote" prefetch={false}>
    ...
    case 'notFound':
      return 'You needed to have uploaded restock sessions before december 2025 to access the Restock Review 2025. Please try again next year.';
    case 'maintenance':
      return 'The 2025 Restock Review is currently disabled for maintenance. Please try again later.';
    default:
