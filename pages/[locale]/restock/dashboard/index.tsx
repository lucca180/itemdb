...
import { getRestockStats } from '@pages/api/v1/restock';
...
const DashboardOptionsModal = dynamic(() => import('@components/Modal/DashboardOptionsModal'));
              ...
              <List.Item>
                <Text>
                  {t.rich('Restock.text-2', {
                    Link: (chunk) => (
                      <Link asChild>
                        <MainLink href="/login" prefetch={false}>
                          {chunk}
                        </MainLink>
                      </Link>
                    ),
            ...
            {showScriptCTA && (
              ...
              >
                ...
                <HStack>
                  ...
                  >
                    <MainLink href="/contribute">{t('General.learn-more')}</MainLink>
                  </Button>
                </HStack>
              </Flex>
            )}
...
export async function getServerSideProps(context: any): Promise<{ props: RestockDashboardProps }> {
  ...
  if (!res || !res.user) {
    return {
      props: {
        tip,
        messages: await loadTranslation(locale as string, 'restock/dashboard/index'),
