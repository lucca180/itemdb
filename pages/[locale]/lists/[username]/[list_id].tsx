...
import { getListMatch } from '@pages/api/v1/lists/match/[...usernames]';
    ...
    try {
      ...
      if (force || !list) {
        ...
        if (listData.slug && listData.slug !== router.query.list_id) {
          router.replace(getPageRouterHref(router, `/lists/${username}/${listData.slug}`));
        }
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const props: ListPageProps = {
    ...
    messages: await loadTranslation(locale as string, 'lists/[username]/[list_id]'),
    locale: locale,
  };
