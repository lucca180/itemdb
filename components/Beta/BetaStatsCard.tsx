...
import { Flex, Heading, VStack, Text, Progress } from "@chakra-ui/react";
    ...
    return (
        <Flex flexFlow='column' mt={5} gap={3} w="100%" maxW="300px">
            <VStack justifyContent={'center'} alignItems="center" w="100%">
                ...
                <Text fontSize='sm' textAlign={'center'}>{stats.itemsTotal}/60000</Text>
            </VStack>
            <VStack justifyContent={'center'} alignItems="center" w="100%">
                <Text color="gray.300">Items Missing info</Text>
                <Progress  w="100%" value={(stats.itemsMissingInfo/stats.itemsTotal)*100}/>
                <Text fontSize='sm' textAlign={'center'}>{stats.itemsMissingInfo}/{stats.itemsTotal}</Text>
            </VStack>
            <VStack justifyContent={'center'} alignItems="center" w="100%">
                <Text color="gray.300">Data Processed</Text>
                <Progress  w="100%" value={100-(stats.itemToProcess/stats.itemProcess)*100}/>
                <Text fontSize='sm' textAlign={'center'}>{stats.itemProcess-stats.itemToProcess}/{stats.itemProcess}</Text>
            </VStack>
            <VStack justifyContent={'center'} alignItems="center" w="100%">
                <Text color="gray.300"><Link href='/feedback/trades'>Trades Priced <ExternalLinkIcon verticalAlign="center"/></Link></Text>
                <Progress  w="100%" value={(stats.tradePricing/stats.tradeTotal)*100}/>
                <Text fontSize='sm' textAlign={'center'}>{stats.tradePricing}/{stats.tradeTotal}</Text>
            </VStack>
        </Flex>
    );
}
